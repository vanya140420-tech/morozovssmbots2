  ...
  return (
    <div className="h-screen bg-[#0A0F1D] text-white flex overflow-hidden font-sans selection:bg-cyan-500/30 relative">
      ...
      {isCookieNoticeOpen && cookieConsent === null && (
        <div className="fixed bottom-0 left-0 right-0 bg-zinc-950/95 backdrop-blur-xl border-t border-zinc-800/80 p-5 z-[200] flex flex-col lg:flex-row gap-5 items-center justify-between shadow-[0_-10px_40px_rgba(0,0,0,0.5)] animate-in slide-in-from-bottom-10">
          <div className="text-[12px] leading-relaxed text-zinc-400 w-full lg:max-w-4xl flex-grow">
          ...
          <div className="flex flex-wrap sm:flex-nowrap gap-2.5 shrink-0 w-full lg:w-auto justify-start lg:justify-end">
             <button onClick={() => { setIsCookieNoticeOpen(false); setIsCookieSettingsOpen(true); }} className="px-4 py-2.5 rounded-xl border border-zinc-700 hover:bg-zinc-800 text-white font-medium transition-colors text-[11px] uppercase tracking-wide whitespace-nowrap flex items-center gap-2"><Settings size={14}/> Налаштування</button>
             <button onClick={handleRejectCookies} className="px-4 py-2.5 rounded-xl border border-zinc-700 hover:bg-zinc-800 text-white font-medium transition-colors text-[11px] uppercase tracking-wide whitespace-nowrap">Лише необхідні</button>
             <button onClick={handleAcceptAllCookies} className="px-6 py-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-zinc-950 font-bold transition-all hover:scale-105 text-[11px] uppercase tracking-wide shadow-[0_0_15px_rgba(34,211,238,0.2)] whitespace-nowrap">Прийняти всі</button>
          </div>
        </div>
      )}
      ...
      {isCookieSettingsOpen && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg bg-[#131B2C] rounded-3xl border border-[#1F2937] shadow-2xl overflow-hidden animate-in zoom-in-95 flex flex-col">
            <div className="px-6 py-5 border-b border-[#1F2937] flex justify-between items-center bg-[#0B1120]">
              <h2 className="text-lg font-bold text-white flex items-center gap-2"><ShieldCheck size={20} className="text-cyan-400"/> Центр управління даними</h2>
              <button onClick={() => setIsCookieSettingsOpen(false)} className="text-gray-400 hover:text-white p-1 rounded-md"><X size={18}/></button>
            </div>
            <div className="p-6 overflow-y-auto space-y-6">
              <div className="bg-[#0B1120] p-4 rounded-xl border border-[#1F2937]">
                <div className="flex justify-between items-start mb-1">
                    <h3 className="font-bold text-white flex items-center gap-2"><Lock size={16} className="text-gray-400"/> Технічно необхідні (Essential)</h3>
                    <span className="text-[10px] font-bold text-green-400 bg-green-500/10 px-2 py-1 rounded uppercase">Завжди активні</span>
                </div>
                <p className="text-xs text-gray-400 mt-2 leading-relaxed">Ці файли cookie абсолютно необхідні для роботи Платформи. Вони забезпечують безпеку авторизації (Firebase Auth), захист від атак та збереження сесії. Вони не збирають особисту інформацію для маркетингу.</p>
              </div>
              
              <div className="bg-[#0B1120] p-4 rounded-xl border border-[#1F2937] transition-colors hover:border-cyan-500/30">
                <div className="flex justify-between items-start">
                    <div>
                        <h3 className="font-bold text-white flex items-center gap-2"><BarChart2 size={16} className="text-blue-400"/> Аналітичні (Analytical)</h3>
                        <p className="text-xs text-gray-400 mt-2 leading-relaxed pr-6">Допомагають нам збирати анонімну статистику (Google Analytics) про те, як ви використовуєте конструктор ботів. Це дозволяє нам знаходити помилки в інтерфейсі та робити сервіс зручнішим.</p>
                    </div>
              ...
              <div className="bg-[#0B1120] p-4 rounded-xl border border-[#1F2937] transition-colors hover:border-cyan-500/30">
                <div className="flex justify-between items-start">
                    <div>
                        <h3 className="font-bold text-white flex items-center gap-2"><Sparkles size={16} className="text-purple-400"/> Маркетингові (Marketing)</h3>
                        <p className="text-xs text-gray-400 mt-2 leading-relaxed pr-6">Використовуються для персоналізації реклами на інших сайтах (ретаргетинг) та вимірювання ефективності наших рекламних кампаній. Якщо вимкнути, ви будете бачити неперсоналізовану рекламу.</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer shrink-0 mt-1">
            ...
            <div className="px-6 py-4 border-t border-[#1F2937] bg-[#0B1120] flex justify-between items-center">
              <button onClick={() => { setCookieTempSettings({ analytical: false, marketing: false }); saveCookieSettings(); }} className="text-xs text-gray-400 hover:text-white font-medium transition-colors">Відхилити необов'язкові</button>
              <button onClick={saveCookieSettings} className="px-6 py-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-zinc-900 font-bold transition-all transform active:scale-95 text-sm shadow-[0_0_15px_rgba(34,211,238,0.3)]">Зберегти вибір</button>
            </div>
          </div>
        </div>
      )}
      ...
      {currentUser && activeTab !== 'landing' && (
        <>
          <header className="md:hidden fixed top-0 left-0 right-0 h-16 bg-[#131B2C]/80 backdrop-blur-md border-b border-[#1F2937] flex items-center justify-between px-4 z-40 w-full">
            <div className="flex items-center gap-3">
              <button onClick={() => setActiveTab('landing')} className="text-gray-500 hover:text-cyan-400 transition-colors p-1.5 bg-[#0B1120] border border-[#1F2937] rounded-lg" title="На сайт">
                <ArrowLeft size={16} />
              </button>
              <div className="flex items-center gap-2 cursor-pointer" onClick={() => { setActiveTab('home'); setIsBuilderOpen(false); }}>
                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-600 to-cyan-400 flex items-center justify-center"><Bot size={18} className="text-white"/></div>
                <span className="font-bold tracking-wider text-sm text-white">MOROZOV</span>
              </div>
            </div>
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="text-gray-400 hover:text-white"><Menu /></button>
          </header>

          <aside className={`fixed md:static inset-y-0 left-0 z-50 bg-[#131B2C] border-r border-[#1F2937] transition-all duration-300 flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} ${isSidebarCollapsed ? 'md:w-20' : 'md:w-64'} w-72 pt-16 md:pt-0`}>
            <div className="hidden md:flex items-center h-20 px-4 border-b border-[#1F2937] shrink-0 w-full relative">
              ...
              <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="hidden md:flex text-gray-500 hover:text-white absolute -right-3 top-7 bg-[#1F2937] rounded-full p-1 border border-[#131B2C] z-10">{isSidebarCollapsed ? <ChevronRight size={14} /> : <ChevronLeft size={14} />}</button>
            </div>
            <div className={`p-6 border-b border-[#1F2937] w-full ${isSidebarCollapsed ? 'md:px-2 md:py-6 flex justify-center' : ''}`}>
            ...
            <nav className="flex-grow overflow-y-auto py-4 px-3 space-y-1 scrollbar-none w-full">
              {[{ id: 'home', icon: Home, label: 'Головна' }, { id: 'bots', icon: Layers, label: 'Мої Воронки' }, { id: 'analytics', icon: TrendingUp, label: 'Аналітика' }, { id: 'pricing', icon: CreditCard, label: 'Тарифи та Оплата' }, { id: 'settings', icon: Settings, label: 'Налаштування' }, ...(currentUser.role === 'founder' || currentUser.role === 'admin' ? [{ id: 'admin', icon: ShieldCheck, label: 'Адмін Панель' }] : [])].map(item => (
                <button key={item.id} onClick={() => handleNavClick(item.id)} title={isSidebarCollapsed ? item.label : ''} className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl transition-all duration-200 ${activeTab === item.id ? 'bg-gradient-to-r from-blue-600/20 to-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 'text-gray-400 hover:bg-[#1E293B] hover:text-white border border-transparent'} ${isSidebarCollapsed ? 'justify-center' : ''}`}>
            ...
            <div className="p-4 border-t border-[#1F2937] w-full">
              <button onClick={handleLogout} className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl text-red-400 hover:bg-red-500/10 transition-colors ${isSidebarCollapsed ? 'justify-center' : ''}`}><LogOut size={20} />{!isSidebarCollapsed && <span className="text-sm font-medium">Вийти</span>}</button>
            </div>
          </aside>

          <main className="flex-grow relative flex flex-col h-screen overflow-y-auto bg-[#0A0F1D] pt-16 md:pt-0 w-full">
            <div className="flex-grow max-w-7xl w-full mx-auto p-4 md:p-8 lg:p-10 pb-10">
              
              {/* TAB: HOME */}
              {activeTab === 'home' && (
                <div className="space-y-6 animate-in fade-in duration-500">
                  <div className="bg-gradient-to-r from-[#131B2C] to-[#0B1120] rounded-3xl p-8 border border-[#1F2937] relative overflow-hidden">
                    <div className="absolute right-0 top-0 w-64 h-64 bg-cyan-500/10 rounded-full blur-[80px]" />
                    <h1 className="text-3xl md:text-4xl font-bold text-white mb-2 relative z-10">Привіт, <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">{currentUser.name}</span></h1>
                    <p className="text-gray-400 max-w-xl relative z-10">Створюй потужні автоворонки. Автоматизуй залучення лідів, прогрів та продажі без написання коду.</p>
                    <button onClick={() => openBuilder()} className="mt-6 bg-cyan-500/10 border border-cyan-500/50 hover:bg-cyan-500/20 text-cyan-400 px-6 py-2.5 rounded-xl font-medium transition-all flex items-center gap-2 relative z-10">
                      <Plus size={18} /> Створити Бота
                    </button>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-[#131B2C] border border-[#1F2937] p-6 rounded-3xl hover:border-cyan-500/30 transition-all group">
                      <div className="w-12 h-12 rounded-2xl bg-[#0A0F1D] flex items-center justify-center text-cyan-400 mb-4 group-hover:scale-110 transition-transform"><Database size={24} /></div>
                      <h3 className="text-lg font-semibold text-white mb-2">Розумні Лід-магніти (Flow Builder)</h3>
                      <p className="text-sm text-gray-400 leading-relaxed">Створюйте ланцюжки з умовами, очікуванням вводу та кнопками розгалуження.</p>
                    </div>
                    <div className="bg-[#131B2C] border border-[#1F2937] p-6 rounded-3xl hover:border-cyan-500/30 transition-all group">
                      <div className="w-12 h-12 rounded-2xl bg-[#0A0F1D] flex items-center justify-center text-cyan-400 mb-4 group-hover:scale-110 transition-transform"><Zap size={24} /></div>
                      <h3 className="text-lg font-semibold text-white mb-2">Базові Автоворонки</h3>
                      <p className="text-sm text-gray-400 leading-relaxed">Швидкі відповіді на слова-тригери для базових тарифів.</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'bots' && (
                <div className="space-y-6 animate-in fade-in duration-500">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <h2 className="text-2xl font-bold text-white">Мої Воронки</h2>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {bots.filter(b => b.userId === currentUser.id).length === 0 ? (
                      <div className="col-span-full bg-[#131B2C] border border-[#1F2937] rounded-3xl p-12 text-center">
                        <div className="w-20 h-20 bg-[#0A0F1D] rounded-full flex items-center justify-center mx-auto mb-4 text-gray-500"><Bot size={40} /></div>
                        <h3 className="text-xl font-semibold text-white mb-2">У вас ще немає ботів</h3>
                        <button onClick={() => openBuilder()} className="text-cyan-400 font-medium hover:text-cyan-300">Створити +</button>
                      </div>
                    ) : (
                      bots.filter(b => b.userId === currentUser.id).map(bot => {
                        ...
                        return (
                        <div key={bot.id} className="bg-[#131B2C] border border-[#1F2937] rounded-3xl p-6 hover:border-cyan-500/20 transition-all flex flex-col justify-between group">
                          <div>
                            <div className="flex justify-between items-start mb-4">
                              <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#0B1120] to-[#1F2937] border border-[#1F2937] flex items-center justify-center shadow-inner group-hover:border-cyan-500/50 transition-colors"><Bot className="text-cyan-400" size={24} /></div>
                                <div>
                                  <h3 className="text-lg font-bold text-white">{bot.name}</h3>
                                  <p className="text-xs text-gray-500 font-mono mt-0.5">{bot.username ? `@${bot.username}` : 'Токен не перевірено'}</p>
                                </div>
                              </div>
                              <button onClick={() => toggleBotStatus(bot.id)} className={`p-2 rounded-xl transition-colors ${bot.status === 'Активний' ? 'bg-amber-500/10 text-amber-400 hover:bg-amber-500/20' : 'bg-green-500/10 text-green-400 hover:bg-green-500/20'}`} title={bot.status === 'Активний' ? 'Призупинити' : 'Запустити'}>{bot.status === 'Активний' ? <Pause size={18} /> : <Play size={18} />}</button>
                            </div>
                            <div className="flex items-center gap-2 mb-6">
                              <span className={`inline-flex items-center gap-1.5 text-[11px] font-medium px-2 py-1 rounded-md ${bot.status === 'Активний' ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>
                                <div className={`w-1.5 h-1.5 rounded-full ${bot.status === 'Активний' ? 'bg-green-400 animate-pulse' : 'bg-amber-400'}`} />{bot.status === 'Активний' ? 'Працює (В мережі)' : 'Пауза'}
                              </span>
                              <div className="w-px h-4 bg-[#1F2937]"></div>
                              <div className="flex gap-1.5 flex-wrap">
                                {bot.modules.map((mod, i) => <span key={i} className="text-[10px] text-blue-300 bg-blue-500/10 border border-blue-500/20 px-2 py-1 rounded-md">{mod}</span>)}
                              </div>
                            </div>
                          </div>
                          <div className="flex justify-between items-end border-t border-[#1F2937] pt-4 mt-auto">
                            <div className="flex gap-6">
                              <div>
                                <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Юзери (Цей місяць)</p>
                                <p className={`text-white font-mono font-medium text-sm ${isLimitReached ? 'text-red-400' : ''}`}>
                                  {currentUsersCount.toLocaleString()} <span className="text-gray-500 text-xs">/ {displayMaxUsers}</span>
                                </p>
                              </div>
                              <div>
                                <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-1">Завершено циклів</p>
                                <p className="text-cyan-400 font-mono font-medium text-sm">{(bot.interactions || 0).toLocaleString()}</p>
                              </div>
                            </div>
                            <button onClick={() => openBuilder(bot)} className="text-sm text-white hover:text-cyan-400 transition-colors bg-[#0B1120] px-4 py-2 rounded-xl border border-[#1F2937] hover:border-cyan-500/30">Налаштувати</button>
                          </div>
                        </div>
                      )})
                    )}
              ...
              {activeTab === 'analytics' && (() => {
                ...
                <div className="space-y-6 animate-in fade-in duration-500">
                  <div className="flex flex-col md:flex-row justify-between md:items-end gap-4 mb-6">
                    <div>
                      <h2 className="text-2xl font-bold text-white mb-2">Аналітика</h2>
                      <p className="text-gray-400 text-sm">Зведення по вашій воронці продажів.</p>
                    </div>
                    <div className="w-full md:w-64">
                      <label className="text-[10px] uppercase text-gray-500 font-bold tracking-wider mb-1.5 block">Оберіть проєкт</label>
                      <div className="relative">
                        <select value={analyticsSelectedBot} onChange={(e) => setAnalyticsSelectedBot(e.target.value)} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-xl pl-4 pr-10 py-2.5 text-white text-sm focus:border-cyan-500 focus:outline-none appearance-none cursor-pointer">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      ...
                      <div key={i} className="bg-[#131B2C] border border-[#1F2937] p-5 rounded-3xl relative overflow-hidden group hover:border-zinc-700 transition-colors">
                  ...
                  <div className="bg-[#131B2C] border border-[#1F2937] rounded-3xl p-6 h-[400px] flex flex-col">
                    <h3 className="text-white font-medium mb-6">Динаміка за тиждень</h3>
                    <div className="flex-grow">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={ANALYTICS_DATA_WEEK} margin={{ top: 5, right: 0, bottom: 5, left: -20 }}>
                          ...
                          <Tooltip contentStyle={{ backgroundColor: '#0B1120', borderColor: '#1F2937', borderRadius: '12px', color: '#fff' }} itemStyle={{ color: '#22D3EE' }} />
              ...
              {activeTab === 'pricing' && (
                <div className="space-y-8 animate-in fade-in duration-500 max-w-5xl mx-auto">
                  <div className="text-center max-w-2xl mx-auto mb-12">
                    <h2 className="text-3xl font-bold text-white mb-4">Оберіть ідеальний тариф</h2>
                    <p className="text-gray-400">Масштабуйте продажі за допомогою автоворонок. Скасуйте в будь-який момент.</p>
                    <div className="inline-flex items-center gap-2 mt-6 bg-[#131B2C] border border-[#1F2937] px-4 py-2 rounded-full text-sm">
                      <span className="text-gray-400">Ваш поточний тариф:</span>
                      <span className={`font-bold ${currentUser.plan === 'Unlimited' ? 'text-amber-400' : currentUser.plan === 'Agency' ? 'text-purple-400' : currentUser.plan === 'Pro' ? 'text-blue-400' : 'text-green-400'}`}>{currentUser.plan}</span>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {['Starter', 'Pro', 'Agency'].map(planKey => (
                      <div key={planKey} className={`${planKey === 'Pro' ? 'bg-gradient-to-b from-[#131B2C] to-[#0A0F1D] border-2 border-cyan-500/50 transform md:-translate-y-4 shadow-[0_0_30px_rgba(34,211,238,0.15)]' : 'bg-[#131B2C] border border-[#1F2937]'} rounded-3xl p-8 flex flex-col relative`}>
                        {planKey === 'Pro' && <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-cyan-500 text-[#0A0F1D] px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider">Популярный</div>}
                        <h3 className={`text-xl font-bold mb-2 ${planKey === 'Starter' ? 'text-white' : planKey === 'Pro' ? 'text-cyan-400' : 'text-white'}`}>{planKey}</h3>
                        <p className="text-gray-400 text-sm mb-6 whitespace-pre-wrap leading-relaxed">{plansConfig[planKey].description}</p>
                        <div className="mb-6 mt-auto"><span className="text-4xl font-bold text-white">₴{plansConfig[planKey].price}</span><span className="text-gray-500"> / міс</span></div>
                        <ul className="space-y-4 mb-8 text-sm text-gray-300">
                          {plansConfig[planKey].features.map((feat, i) => (
                            <li key={i} className={`flex items-center gap-3 ${feat.included ? '' : 'text-gray-600'}`}>
                              {feat.included ? <Check size={16} className={planKey === 'Starter' ? 'text-green-400' : planKey === 'Pro' ? 'text-cyan-400' : 'text-purple-400'}/> : <X size={16} />}
                              {feat.text}
                            </li>
                          ))}
                        </ul>
                        {currentUser.plan === planKey || (currentUser.plan === 'Unlimited' && planKey === 'Agency') ? (
                          <button disabled className={`w-full py-3 font-medium rounded-xl border cursor-not-allowed ${planKey === 'Pro' ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/30' : planKey === 'Agency' ? 'bg-purple-500/10 text-purple-400 border-purple-500/20' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}>Поточний тариф</button>
                        ) : (
                          <button onClick={() => { setCheckoutPlan(planKey); setCheckoutAgreed(false); }} className={`w-full py-3 font-medium rounded-xl transition-all ${planKey === 'Pro' ? 'bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white font-bold shadow-[0_0_15px_rgba(34,211,238,0.4)] transform active:scale-[0.98]' : 'bg-[#1E293B] hover:bg-[#2D3748] text-white border border-transparent ' + (planKey === 'Agency' ? 'hover:border-purple-500' : '')}`}>Обрати {planKey}</button>
                        )}
                      </div>
                    ))}
                  </div>
              ...
              {/* TAB: SETTINGS (Профіль) */}
              {activeTab === 'settings' && (
                <div className="space-y-6 animate-in fade-in duration-500 max-w-3xl">
                  <h2 className="text-2xl font-bold text-white mb-6">Налаштування профілю</h2>
                  <form onSubmit={saveSettings} className="space-y-6">
                    <div className="bg-[#131B2C] border border-[#1F2937] p-6 rounded-3xl">
                      <div className="flex items-center gap-3 mb-6 pb-4 border-b border-[#1F2937]">
                        <User size={20} className="text-cyan-400"/>
                        <h3 className="text-lg font-semibold text-white">Обліковий запис</h3>
                      </div>
                      
                      <div className="grid grid-cols-1 gap-5">
                        <div className="bg-[#0B1120] border border-[#1F2937] rounded-xl p-4 flex items-center justify-between mb-2">
                          <div>
                            <p className="text-xs text-gray-400 uppercase tracking-wider font-bold mb-1">Ваш унікальний ID</p>
                            <p className="text-sm font-mono text-cyan-400">{currentUser.id}</p>
                          </div>
                          <Fingerprint size={24} className="text-gray-600" />
                        </div>
                        
                        <div className="space-y-1.5">
                          <label className="text-xs text-gray-400 uppercase tracking-wider font-bold block">Ім'я</label>
                          <input type="text" value={settingsForm.name} onChange={e=>setSettingsForm({...settingsForm, name: e.target.value})} required className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-3 text-white focus:border-cyan-500 focus:outline-none transition-colors shadow-sm" placeholder="Ваше ім'я" />
                        </div>
                        <div className="space-y-1.5">
                          <label className="text-xs text-gray-400 uppercase tracking-wider font-bold block">Змінити пароль</label>
                          <input type="password" value={settingsForm.password} onChange={e=>setSettingsForm({...settingsForm, password: e.target.value})} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-3 text-white focus:border-cyan-500 focus:outline-none transition-colors shadow-sm" placeholder="Введіть новий пароль" />
                        </div>
                      </div>
                    </div>
                    <div className="flex justify-end pt-4">
                      <button type="submit" className="bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white font-bold py-3 px-8 rounded-xl shadow-[0_0_15px_rgba(34,211,238,0.3)] transition-all">Зберегти налаштування</button>
                    </div>
                  </form>

                  {/* Управління підпискою */}
                  <div className="bg-[#131B2C] border border-[#1F2937] p-6 rounded-3xl mt-6">
                    <div className="flex items-center gap-3 mb-6 pb-4 border-b border-[#1F2937]">
                      <CardIcon size={20} className="text-cyan-400"/>
                      <h3 className="text-lg font-semibold text-white">Управління підпискою</h3>
                    </div>
                    
                    <div className="bg-[#0B1120] border border-[#1F2937] rounded-xl p-5 mb-4">
                      <div className="flex flex-col md:flex-row justify-between md:items-center gap-2 mb-2">
                          <span className="text-gray-400 text-sm">Поточний тариф:</span>
                          <span className={`font-bold ${currentUser.plan === 'Unlimited' ? 'text-amber-400' : currentUser.plan === 'Agency' ? 'text-purple-400' : currentUser.plan === 'Pro' ? 'text-blue-400' : 'text-green-400'}`}>{currentUser.plan}</span>
                      </div>
                      {currentUser.plan !== 'Starter' && currentUser.plan !== 'Unlimited' && (
                          <div className="flex flex-col md:flex-row justify-between md:items-center gap-2 pt-2 border-t border-[#1F2937]/50 mt-2">
                              <span className="text-gray-400 text-sm">Автосписання (Автопродовження):</span>
                              {currentUser.autoRenew !== false ? (
                                  <span className="text-green-400 text-[11px] uppercase tracking-wider font-bold bg-green-500/10 border border-green-500/20 px-2.5 py-1 rounded-md self-start md:self-auto">Увімкнено</span>
                              ) : (
                                  <span className="text-amber-400 text-[11px] uppercase tracking-wider font-bold bg-amber-500/10 border border-amber-500/20 px-2.5 py-1 rounded-md self-start md:self-auto">Вимкнено</span>
                              )}
                          </div>
                      )}
                      {currentUser.refundRequested && (
                          <div className="mt-4 bg-amber-500/10 border border-amber-500/20 p-3 rounded-xl flex items-start gap-3">
                              ...
                              <p className="text-xs text-amber-400/90 leading-relaxed">Ваш запит на повернення коштів знаходиться на розгляді. Ми зв'яжемося з вами найближчим часом через пошту.</p>
                          </div>
                      )}
                    </div>
                    ...
                    {currentUser.plan !== 'Starter' && currentUser.plan !== 'Unlimited' && (
                        <div className="flex flex-col sm:flex-row gap-3">
                            {currentUser.autoRenew !== false && (
                                <button onClick={handleCancelAutoRenew} className="flex-1 py-3 rounded-xl border border-[#1F2937] text-gray-300 hover:text-white hover:bg-[#1E293B] font-medium transition-colors text-sm">
                                    Вимкнути автопродовження
                                </button>
                            )}
                            {!currentUser.refundRequested && (
                                <button onClick={handleRequestRefund} className="flex-1 py-3 rounded-xl border border-red-500/30 text-red-400 hover:text-red-300 hover:bg-red-500/10 font-medium transition-colors text-sm">
                                    Запросити Refund (Повернення)
                                </button>
                            )}
                        </div>
                    )}
                    {(currentUser.plan === 'Starter' || currentUser.plan === 'Unlimited') && (
                        <p className="text-xs text-gray-500 text-center mt-2">Для вашого поточного тарифу управління автоматичним списанням не застосовується.</p>
                    )}
                  </div>
                </div>
              )}

              {/* ADMIN TAB: SUPER ADMIN PANEL */}
              {activeTab === 'admin' && ['founder', 'admin'].includes(currentUser.role) && (
                <div className="space-y-6 animate-in fade-in duration-500">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                    <div className="flex items-center gap-3">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center border ${currentUser.role === 'founder' ? 'bg-red-500/10 text-red-500 border-red-500/20' : 'bg-purple-500/10 text-purple-500 border-purple-500/20'}`}><ShieldCheck size={24}/></div>
                      <div>
                        <h2 className="text-2xl font-bold text-white leading-tight">{currentUser.role === 'founder' ? 'SuperAdmin' : 'Адміністратор'}</h2>
                        <p className="text-sm text-gray-400">Управління платформою</p>
                      </div>
                    </div>
                    
                    <div className="flex items-center gap-3 flex-wrap">
                      <div className="flex bg-[#131B2C] border border-[#1F2937] rounded-xl p-1">
                        <button onClick={() => setAdminSubTab('users')} className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${adminSubTab === 'users' ? 'bg-cyan-500/20 text-cyan-400' : 'text-gray-400 hover:text-white'}`}>Користувачі</button>
                        <button onClick={() => setAdminSubTab('funnels')} className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${adminSubTab === 'funnels' ? 'bg-cyan-500/20 text-cyan-400' : 'text-gray-400 hover:text-white'}`}>Базові Автоворонки</button>
                        <button onClick={() => setAdminSubTab('leadmagnets')} className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${adminSubTab === 'leadmagnets' ? 'bg-cyan-500/20 text-cyan-400' : 'text-gray-400 hover:text-white'}`}>Лід-магніти</button>
                        {currentUser.role === 'founder' && (
                          <button onClick={() => setAdminSubTab('settings')} className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${adminSubTab === 'settings' ? 'bg-cyan-500/20 text-cyan-400' : 'text-gray-400 hover:text-white'}`}>Налаштування Платформи</button>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  {adminSubTab === 'users' && (
                    <>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <div className="bg-[#131B2C] border border-[#1F2937] p-5 rounded-2xl">
                          <p className="text-xs text-gray-400 mb-1">Всього користувачів</p>
                          <p className="text-2xl font-bold font-mono text-white">{users.length}</p>
                        </div>
                        <div className="bg-[#131B2C] border border-[#1F2937] p-5 rounded-2xl border-l-4 border-l-green-500">
                          <p className="text-xs text-gray-400 mb-1">Очікуваний MRR (Дохід)</p>
                      ...
                      <div className="mb-6 w-full relative">
                        ...
                        <input 
                          ...
                          placeholder="Пошук користувачів за ID, Ім'ям або Тарифом..." 
                          className="w-full bg-[#131B2C] border border-[#1F2937] rounded-xl pl-10 pr-4 py-3 text-sm text-white focus:border-cyan-500 outline-none transition-colors"
                        />
                      </div>

                      <div className="bg-[#131B2C] border border-[#1F2937] rounded-3xl overflow-hidden">
                        <div className="overflow-x-auto">
                          <table className="w-full text-left text-sm">
                            <thead className="bg-[#0B1120] text-gray-400 text-xs uppercase tracking-wider border-b border-[#1F2937]">
                              <tr>
                                ...
                                <tr key={u.id} className="hover:bg-white/5 transition-colors">
                                  <td className="px-6 py-4">
                                    <div className="font-medium text-white flex items-center gap-2">
                                        ...
                                        {u.status && !u.status.includes('Бан') && !u.status.includes('Starter') && !u.status.includes('Founder') && <span className="text-cyan-400 text-[10px] uppercase font-bold tracking-wider ml-1">{u.status}</span>}
                                    </div>
                                    <div className="text-[10px] text-cyan-500 mt-0.5">{u.id}</div>
                                  </td>
                                  <td className="px-6 py-4">
                                    <span className={`inline-flex px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${u.role === 'founder' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : u.role === 'admin' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' : 'bg-gray-800 text-gray-300'}`}>
                                      {u.role}
                                    </span>
                                  </td>
                                  <td className="px-6 py-4">
                                    <div className="text-gray-300 font-medium">{u.plan}</div>
                                    {u.planStartDate && <div className="text-[10px] text-green-400 mt-1 uppercase tracking-wider font-bold">Оплачено: {new Date(u.planStartDate).toLocaleDateString('uk-UA')}</div>}
                                    {u.planExpiry && <div className="text-[10px] text-gray-500 mt-0.5 uppercase tracking-wider font-bold">Діє до: {u.planExpiry === '2099-12-31' ? 'Назавжди' : new Date(u.planExpiry).toLocaleDateString('uk-UA')}</div>}
                                  </td>
                                  <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end gap-2">
                                      {(currentUser.role === 'founder' || u.role !== 'founder') && (
                                        <button onClick={() => setAdminEditingUser({...u, newPassword: ''})} className="text-blue-400 hover:text-blue-300 hover:bg-blue-500/10 p-1.5 rounded-lg transition-colors" title="Редагувати">
                                          <Edit size={16} />
                                        </button>
                                      )}
                                      {u.id !== currentUser.id && u.id !== 'ID0' && (currentUser.role === 'founder' || u.role !== 'founder') && (
                                        <>
                                          <button onClick={() => adminPauseUserBots(u.id)} className="text-amber-400 hover:text-amber-300 hover:bg-amber-500/10 p-1.5 rounded-lg transition-colors" title="Зупинити всіх ботів користувача">
                                            <Pause size={16} />
                                          </button>
                                          <button onClick={() => adminDeleteUser(u.id)} className="text-red-400 hover:text-red-300 hover:bg-red-500/10 p-1.5 rounded-lg transition-colors" title="Видалити">
                                            <Trash2 size={16} />
                                          </button>
                                        </>
                                      )}
                  ...
                  {/* ADMIN TAB: BOTS (Автоворонки) */}
                  {adminSubTab === 'funnels' && (
                    <>
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                        <div className="bg-[#131B2C] border border-[#1F2937] p-5 rounded-2xl flex-shrink-0">
                          <p className="text-xs text-gray-400 mb-1">Всього базових автоворонок</p>
                          <p className="text-2xl font-bold font-mono text-cyan-400">{bots.filter(b => b.modules.includes('Автоворонка')).length}</p>
                        </div>
                      ...
                      <div className="bg-[#131B2C] border border-[#1F2937] rounded-3xl overflow-hidden">
                        <div className="overflow-x-auto">
                          <table className="w-full text-left text-sm">
                            <thead className="bg-[#0B1120] text-gray-400 text-xs uppercase tracking-wider border-b border-[#1F2937]">
                              <tr>
                                <th className="px-6 py-4 font-medium">Воронка</th>
                                <th className="px-6 py-4 font-medium">Власник</th>
                                <th className="px-6 py-4 font-medium">Статус</th>
                                <th className="px-6 py-4 font-medium text-right">Дії (God Mode)</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-[#1F2937]">
                                  ...
                                  <tr key={b.id} className="hover:bg-white/5 transition-colors">
                                    <td className="px-6 py-4">
                                      ...
                                      <div className="text-[10px] text-gray-400 mt-1">
                                        Юзерів (місяць): {b.uniqueUserIds?.length || 0} / {getPlanLimitForUser(owner, 'maxUsers') === Infinity ? '∞' : getPlanLimitForUser(owner, 'maxUsers')}
                                      </div>
                                    </td>
                                    <td className="px-6 py-4">
                                      <div className="text-gray-300">{owner ? owner.name : 'Видалено'}</div>
                                      <div className="text-[10px] text-cyan-500 mt-0.5">{b.userId}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                      <span className={`inline-flex px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${b.status === 'Активний' ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>
                                        {b.status}
                                      </span>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                      <div className="flex justify-end gap-2">
                                        <button onClick={() => openBuilder(b)} className="text-cyan-400 hover:text-cyan-300 hover:bg-cyan-500/10 p-1.5 rounded-lg transition-colors flex items-center gap-1 text-xs font-medium" title="Увійти та редагувати чужого бота">
                                          <Settings size={14} /> Налаштувати
                                        </button>
                                        <button onClick={() => toggleBotStatus(b.id)} className="text-gray-400 hover:text-white hover:bg-white/10 p-1.5 rounded-lg transition-colors" title={b.status === 'Активний' ? 'Призупинити' : 'Запустити'}>
                                          {b.status === 'Активний' ? <Pause size={16} /> : <Play size={16} />}
                                        </button>
                                        <button onClick={() => adminDeleteBot(b.id)} className="text-red-400 hover:text-red-300 hover:bg-red-500/10 p-1.5 rounded-lg transition-colors" title="Видалити">
                                          <Trash2 size={16} />
                                        </button>
                                      </div>
                                    </td>
                  ...
                  {/* ADMIN TAB: BOTS (Лід-магніти) */}
                  {adminSubTab === 'leadmagnets' && (
                    <>
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                        <div className="bg-[#131B2C] border border-[#1F2937] p-5 rounded-2xl flex-shrink-0">
                          <p className="text-xs text-gray-400 mb-1">Всього розумних лід-магнітів</p>
                          <p className="text-2xl font-bold font-mono text-cyan-400">{bots.filter(b => b.modules.includes('Лід-магніт')).length}</p>
                        </div>
                        <div className="w-full md:w-96 relative">
                          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <Search size={18} className="text-gray-500" />
                      ...
                      <div className="bg-[#131B2C] border border-[#1F2937] rounded-3xl overflow-hidden">
                        <div className="overflow-x-auto">
                          <table className="w-full text-left text-sm">
                            <thead className="bg-[#0B1120] text-gray-400 text-xs uppercase tracking-wider border-b border-[#1F2937]">
                              <tr>
                                <th className="px-6 py-4 font-medium">Лід-магніт</th>
                                <th className="px-6 py-4 font-medium">Власник</th>
                                <th className="px-6 py-4 font-medium">Статус</th>
                                <th className="px-6 py-4 font-medium text-right">Дії (God Mode)</th>
                              </tr>
                  ...
                  {/* ADMIN TAB: PLATFORM SETTINGS */}
                  {adminSubTab === 'settings' && (
                    <div className="space-y-6 pb-20">
                      <div className="bg-[#131B2C] border border-[#1F2937] p-6 rounded-3xl mb-6">
                        <div className="flex justify-between items-center mb-6">
                            <div>
                                <h3 className="text-lg font-bold text-white mb-1">Юридична інформація (Оферти)</h3>
                                <p className="text-sm text-gray-400">Ці дані автоматично підставляться у всі юридичні документи.</p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2 bg-[#0B1120] p-5 rounded-2xl border border-[#1F2937]">
                            <div>
                                <label className="text-[10px] text-gray-500 uppercase font-bold block mb-1">ФОП / ТОВ</label>
                                <input type="text" value={companyInfo.fop} onChange={(e) => setCompanyInfo({...companyInfo, fop: e.target.value})} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" placeholder="ФОП Іванов І.І." />
                            </div>
                            <div>
                                <label className="text-[10px] text-gray-500 uppercase font-bold block mb-1">Код ЄДРПОУ / ІПН</label>
                                <input type="text" value={companyInfo.edrpou} onChange={(e) => setCompanyInfo({...companyInfo, edrpou: e.target.value})} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" placeholder="1234567890" />
                            </div>
                            <div className="md:col-span-2">
                                <label className="text-[10px] text-gray-500 uppercase font-bold block mb-1">Юридична адреса</label>
                                <input type="text" value={companyInfo.address} onChange={(e) => setCompanyInfo({...companyInfo, address: e.target.value})} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" placeholder="м. Київ, вул. Хрещатик, 1" />
                            </div>
                            <div>
                                <label className="text-[10px] text-gray-500 uppercase font-bold block mb-1">Email</label>
                                <input type="text" value={companyInfo.email} onChange={(e) => setCompanyInfo({...companyInfo, email: e.target.value})} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" placeholder="support@morozov.com" />
                            </div>
                            <div>
                                <label className="text-[10px] text-gray-500 uppercase font-bold block mb-1">Telegram Support</label>
                                <input type="text" value={companyInfo.tgSupport} onChange={(e) => setCompanyInfo({...companyInfo, tgSupport: e.target.value})} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" placeholder="@morozov_support" />
                            </div>
                        </div>

                        <div className="flex justify-between items-center mb-6 mt-8">
                            <div>
                                <h3 className="text-lg font-bold text-white mb-1">Налаштування Тарифів</h3>
                                <p className="text-sm text-gray-400">Зміни тут миттєво відобразяться на лімітах та сторінці "Тарифи та Оплата".</p>
                            </div>
                        </div>
                        
                        <div className="grid grid-cols-1 gap-6">
                          {['Starter', 'Pro', 'Agency'].map(planKey => (
                            <div key={planKey} className="bg-[#0B1120] border border-[#1F2937] p-5 rounded-2xl flex flex-col lg:flex-row gap-6">
                              
                              <div className="flex-1 space-y-4">
                                  <h4 className={`text-xl font-bold ${planKey === 'Starter' ? 'text-green-400' : planKey === 'Pro' ? 'text-cyan-400' : 'text-purple-400'}`}>{planKey}</h4>
                                  
                                  <div>
                                    <label className="text-[10px] text-gray-500 uppercase font-bold block mb-1">Короткий опис</label>
                                    <textarea rows="3" value={plansConfig[planKey].description} onChange={(e) => adminUpdatePlans(planKey, 'description', e.target.value)} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-xs outline-none focus:border-cyan-500 resize-none whitespace-pre-wrap" />
                                  </div>
                                  <div className="grid grid-cols-2 gap-4">
                                      <div>
                                          <label className="text-[10px] text-gray-500 uppercase font-bold block mb-1">Ціна (₴)</label>
                                          <input type="number" value={plansConfig[planKey].price} onChange={(e) => adminUpdatePlans(planKey, 'price', Number(e.target.value))} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" />
                                      </div>
                                      <div>
                                          <label className="text-[10px] text-gray-500 uppercase font-bold block mb-1">Макс. Ботів</label>
                                          <input type="text" value={plansConfig[planKey].maxBots} onChange={(e) => adminUpdatePlans(planKey, 'maxBots', e.target.value)} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" />
                                      </div>
                                      <div>
                                          <label className="text-[10px] text-gray-500 uppercase font-bold block mb-1">Макс. Юзерів</label>
                                          <input type="text" value={plansConfig[planKey].maxUsers} onChange={(e) => adminUpdatePlans(planKey, 'maxUsers', e.target.value)} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" />
                                      </div>
                                      <div>
                                          <label className="text-[10px] text-gray-500 uppercase font-bold block mb-1">Макс. Воронок</label>
                                          <input type="text" value={plansConfig[planKey].maxFlows} onChange={(e) => adminUpdatePlans(planKey, 'maxFlows', e.target.value)} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" />
                                      </div>
                                      <div className="col-span-2">
                                          ...
                                          <input type="text" value={plansConfig[planKey].maxModules} onChange={(e) => adminUpdatePlans(planKey, 'maxModules', e.target.value)} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" />
                                      </div>
                                      <div className="col-span-2">
                                          <label className="text-[10px] text-gray-500 uppercase font-bold block mb-1">Посилання на оплату (WayForPay URL)</label>
                                          <input type="url" value={plansConfig[planKey].paymentUrl || ''} onChange={(e) => adminUpdatePlans(planKey, 'paymentUrl', e.target.value)} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" placeholder="https://secure.wayforpay.com/button/..." />
                                      </div>
                                  </div>
                                  ...
                                  <div className="pt-2 border-t border-[#1F2937]">
                                    ...
                                    <div className="flex gap-4">
                                      <label className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer">
                                          <input type="checkbox" checked={plansConfig[planKey].allowedModules?.includes('Автоворонка') || false} onChange={() => adminTogglePlanModule(planKey, 'Автоворонка')} className="accent-cyan-500 w-4 h-4" /> Автоворонка
                                      </label>
                                      <label className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer">
                                          <input type="checkbox" checked={plansConfig[planKey].allowedModules?.includes('Лід-магніт') || false} onChange={() => adminTogglePlanModule(planKey, 'Лід-магніт')} className="accent-cyan-500 w-4 h-4" /> Розумний Лід-магніт
                                      </label>
                                    </div>
                                  </div>
                              </div>
                                ...
                                <div className="space-y-2 mb-3 flex-grow">
                                  {plansConfig[planKey].features.map((feat, idx) => (
                                    <div key={idx} className="flex gap-2 items-center">
                                      ...
                                      <input type="text" value={feat.text} onChange={(e) => adminUpdateFeatureText(planKey, idx, e.target.value)} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-lg px-2 py-1.5 text-white text-xs outline-none focus:border-cyan-500" />
                                      <button onClick={() => adminRemoveFeature(planKey, idx)} className="shrink-0 w-6 h-6 flex items-center justify-center text-gray-500 hover:text-red-400 transition-colors"><Trash2 size={12}/></button>
                                    </div>
                                  ))}
                                </div>
                                <button onClick={() => adminAddFeature(planKey)} className="w-full py-2 bg-blue-600/10 text-cyan-400 text-xs rounded-lg border border-cyan-500/20 font-medium hover:bg-blue-600/20 transition-colors mt-auto">+ Додати опцію</button>
                              </div>
      ...
      {/* --- PAYMENT CHECKOUT MODAL (WayForPay Style) --- */}
      {checkoutPlan && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => !isProcessingPayment && setCheckoutPlan(null)} />
          <div className="relative w-full max-w-md bg-[#131B2C] rounded-3xl border border-[#1F2937] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.7)] overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-[#1F2937] flex justify-between items-center bg-[#0B1120]">
              <h2 className="text-lg font-bold text-white flex items-center gap-2"><CardIcon size={18} className="text-cyan-400"/> Оформлення підписки</h2>
              <button disabled={isProcessingPayment} onClick={() => setCheckoutPlan(null)} className="text-gray-400 hover:text-white p-1 rounded-md transition-colors disabled:opacity-50"><X size={18}/></button>
            </div>
            <div className="p-6">
              <div className="bg-[#0B1120] border border-[#1F2937] rounded-2xl p-6 text-center mb-6 relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-cyan-500/10 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2"></div>
                  <h3 className="text-xl font-bold text-white mb-2">Тариф {checkoutPlan}</h3>
                  <p className="text-sm text-gray-400 mb-4">Для завершення оплати вас буде перенаправлено на безпечну сторінку WayForPay.</p>
                  <div className="text-4xl font-black text-white">₴{plansConfig[checkoutPlan]?.price}</div>
              </div>

              <div className="mt-4 mb-3 flex items-start gap-3 bg-[#0B1120] border border-[#1F2937] p-3 rounded-xl">
                  <input type="checkbox" id="checkoutAgreed" checked={checkoutAgreed} onChange={e => setCheckoutAgreed(e.target.checked)} className="mt-1 w-4 h-4 accent-cyan-500 flex-shrink-0 cursor-pointer" />
                  <label htmlFor="checkoutAgreed" className="text-[11px] text-gray-400 leading-snug cursor-pointer select-none">
                      Я погоджуюсь з умовами <span onClick={(e)=>{e.preventDefault(); e.stopPropagation(); toggleModal('offer', true);}} className="text-cyan-400 hover:underline">Публічної оферти</span>, <span onClick={(e)=>{e.preventDefault(); e.stopPropagation(); toggleModal('refund', true);}} className="text-cyan-400 hover:underline">Політикою повернення коштів</span> та підтверджую оплату підписки.
                  </label>
              </div>
              
              <button onClick={handleWayForPayCheckout} disabled={isProcessingPayment || !checkoutAgreed} className="w-full py-4 rounded-xl text-[#0A0F1D] bg-gradient-to-r from-cyan-400 to-blue-500 font-bold shadow-[0_0_20px_rgba(34,211,238,0.3)] hover:scale-[1.02] transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:hover:scale-100 disabled:cursor-not-allowed">
                  {isProcessingPayment ? <Loader2 size={20} className="animate-spin" /> : <>Оплатити через WayForPay <ArrowRight size={18}/></>}
              </button>
              <div className="mt-4 flex items-center justify-center gap-2 text-[11px] text-gray-500 font-medium">
                  <ShieldCheck size={14} className="text-green-500/70" /> Платежі безпечно обробляються WayForPay
              </div>
            </div>
          </div>
        </div>
      ...
      {adminEditingUser && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-[#131B2C] rounded-3xl border border-[#1F2937] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.7)] overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-[#1F2937] flex justify-between items-center bg-[#0B1120]">
              <h2 className="text-lg font-bold text-white flex items-center gap-2"><Edit size={18} className="text-cyan-400"/> Редагувати користувача</h2>
              <button onClick={() => setAdminEditingUser(null)} className="text-gray-400 hover:text-white p-1 rounded-md transition-colors"><X size={18}/></button>
            </div>
            <form onSubmit={adminSaveUser} className="p-6 space-y-4">
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Ім'я</label>
                <input type="text" value={adminEditingUser.name} onChange={e => setAdminEditingUser({...adminEditingUser, name: e.target.value})} required className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-2.5 text-white focus:border-cyan-500 outline-none" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Кастомний статус</label>
                <input type="text" value={adminEditingUser.status || ''} onChange={e => setAdminEditingUser({...adminEditingUser, status: e.target.value})} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-2.5 text-white focus:border-cyan-500 outline-none" placeholder="VIP, Очікує оплати, Заблокований..." />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Роль</label>
                  <select value={adminEditingUser.role} onChange={e => setAdminEditingUser({...adminEditingUser, role: e.target.value})} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-2.5 text-white focus:border-cyan-500 outline-none cursor-pointer">
                ...
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Тариф</label>
                  <select value={adminEditingUser.plan} onChange={e => setAdminEditingUser({...adminEditingUser, plan: e.target.value})} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-2.5 text-white focus:border-cyan-500 outline-none cursor-pointer">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Дата оплати (Початок)</label>
                  <input type="date" value={adminEditingUser.planStartDate || ''} onChange={e => setAdminEditingUser({...adminEditingUser, planStartDate: e.target.value})} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-2.5 text-white focus:border-cyan-500 outline-none" />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Термін дії (Кінець)</label>
                  <input type="date" value={adminEditingUser.planExpiry || ''} onChange={e => setAdminEditingUser({...adminEditingUser, planExpiry: e.target.value})} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-2.5 text-white focus:border-cyan-500 outline-none" />
                </div>
              </div>
              <div className="pt-4 flex justify-end gap-2">
                <button type="button" onClick={() => setAdminEditingUser(null)} className="px-4 py-2.5 rounded-xl text-gray-400 hover:text-white hover:bg-white/5 transition-colors text-sm font-medium">Скасувати</button>
                <button type="submit" className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-bold transition-all hover:scale-105 shadow-[0_0_15px_rgba(34,211,238,0.3)] text-sm">Зберегти</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {isBuilderOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => !isSaving && setIsBuilderOpen(false)} />
          <div className="relative w-full max-w-5xl h-[90vh] bg-[#0B1120] rounded-3xl border border-[#1F2937] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.7)] overflow-hidden flex flex-col md:flex-row animate-in zoom-in-95 duration-200">
            <div className="w-full md:w-64 bg-[#131B2C] border-b md:border-b-0 md:border-r border-[#1F2937] p-4 flex flex-row md:flex-col gap-2 shrink-0 overflow-x-auto md:overflow-visible">
              ...
              {[ { id: 'basic', label: 'Основні налаштування', icon: Settings }, { id: 'modules', label: 'Розумні модулі', icon: Zap }, { id: 'menu', label: 'Меню та Чат', icon: LayoutList } ].map(tab => (
                <button key={tab.id} onClick={() => { setBuilderTab(tab.id); setActiveConfigModule(null); setActiveFlowId(null); }} className={`px-4 py-3 rounded-xl text-sm font-medium transition-all text-left whitespace-nowrap flex items-center gap-3 ${builderTab === tab.id ? 'bg-gradient-to-r from-blue-600/20 to-cyan-500/10 text-cyan-400 border border-cyan-500/30 shadow-inner' : 'text-gray-400 hover:bg-[#1E293B] hover:text-white border border-transparent'}`}><tab.icon size={16} />{tab.label}</button>
              ))}
            </div>
            <div className="flex-grow flex flex-col h-full overflow-hidden bg-[#0A0F1D]">
              <div className="p-6 border-b border-[#1F2937] flex justify-between items-center bg-[#0B1120] shrink-0">
                <h2 className="text-xl font-bold text-white flex items-center gap-3">{activeConfigModule && <button onClick={() => { setActiveConfigModule(null); setActiveFlowId(null); }} className="text-gray-400 hover:text-white bg-[#131B2C] p-1.5 rounded-lg border border-[#1F2937]"><ArrowLeft size={16}/></button>} {activeConfigModule ? `Налаштування: ${activeConfigModule}` : (editingBot ? 'Налаштування бота' : 'Створення нового бота')}</h2>
                <button disabled={isSaving} onClick={() => setIsBuilderOpen(false)} className="text-gray-400 hover:text-white bg-[#131B2C] p-2 rounded-full border border-[#1F2937] disabled:opacity-50"><X size={18}/></button>
              </div>
              ...
              <div className="p-6 flex-grow overflow-y-auto">
                {builderTab === 'basic' && (
                  <div className="space-y-6 max-w-xl animate-in fade-in duration-300">
                    <div className="bg-blue-900/10 p-4 rounded-xl border border-blue-500/20 flex gap-3">
                       ...
                       <p className="text-[12px] text-gray-300 leading-relaxed">Підключаючи Telegram-бота, ви підтверджуєте законне право на Token та згоду з Telegram API Policy.</p>
                    </div>

                    {isModuleAllowed(currentUser.plan, 'Автоворонка') && (
                      <div className="space-y-3 p-5 bg-[#131B2C] rounded-2xl border border-[#1F2937]">
                        <label className="text-sm font-bold text-white flex items-center justify-between">
                          API Токен (Автоворонка)
                          {tokenStatusFunnel === 'success' && <span className="text-xs text-green-400 flex items-center gap-1"><CheckCircle2 size={12}/> Перевірено</span>}
                        </label>
                        <div className="flex gap-2">
                          <input type="text" value={builderForm.tokenFunnel} onChange={e => {setBuilderForm({...builderForm, tokenFunnel: e.target.value}); setTokenStatusFunnel('idle');}} className={`flex-grow bg-[#0B1120] border rounded-xl px-4 py-3 text-white font-mono focus:outline-none transition-colors ${tokenStatusFunnel === 'error' ? 'border-red-500/50' : tokenStatusFunnel === 'success' ? 'border-green-500/50' : 'border-[#1F2937] focus:border-cyan-500'}`} placeholder="Токен..." />
                          <button onClick={() => verifyTelegramToken('funnel')} disabled={!builderForm.tokenFunnel || tokenStatusFunnel === 'loading'} className="bg-[#1E293B] hover:bg-[#2D3748] border border-[#334155] text-white px-4 rounded-xl text-sm font-medium transition-colors disabled:opacity-50 flex items-center gap-2">
                    ...
                    {isModuleAllowed(currentUser.plan, 'Лід-магніт') && (
                      <div className="space-y-3 p-5 bg-[#131B2C] rounded-2xl border border-[#1F2937]">
                        <label className="text-sm font-bold text-white flex items-center justify-between">
                          ...
                          {tokenStatusLm === 'success' && <span className="text-xs text-green-400 flex items-center gap-1"><CheckCircle2 size={12}/> Перевірено</span>}
                        </label>
                        <div className="flex gap-2">
                          <input type="text" value={builderForm.tokenLm} onChange={e => {setBuilderForm({...builderForm, tokenLm: e.target.value}); setTokenStatusLm('idle');}} className={`flex-grow bg-[#0B1120] border rounded-xl px-4 py-3 text-white font-mono focus:outline-none transition-colors ${tokenStatusLm === 'error' ? 'border-red-500/50' : tokenStatusLm === 'success' ? 'border-green-500/50' : 'border-[#1F2937] focus:border-cyan-500'}`} placeholder="Токен..." />
                          <button onClick={() => verifyTelegramToken('lm')} disabled={!builderForm.tokenLm || tokenStatusLm === 'loading'} className="bg-[#1E293B] hover:bg-[#2D3748] border border-[#334155] text-white px-4 rounded-xl text-sm font-medium transition-colors disabled:opacity-50 flex items-center gap-2">
                    ...
                    {verifiedBotData && (
                      <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-xl p-4 flex items-center gap-4 mt-2">
                        ...
                        <div>
                          <p className="text-sm text-white font-bold">{verifiedBotData.first_name}</p>
                          <p className="text-xs text-cyan-400 font-mono">@{verifiedBotData.username}</p>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="space-y-2">
                      ...
                      <input type="text" value={builderForm.name} onChange={e=>setBuilderForm({...builderForm, name: e.target.value})} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-xl px-4 py-3 text-white focus:border-cyan-500 focus:outline-none" placeholder="Мой бот..." />
                    </div>
                  </div>
                )}
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {(() => {
                          ...
                          return availableMods.map(mod => (
                            <div key={mod.name} className="relative flex flex-col p-5 rounded-2xl border-2 transition-all bg-[#131B2C] border-cyan-500/50 shadow-[0_0_20px_rgba(34,211,238,0.1)]">
                              <div className="flex justify-between items-start mb-3"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl flex items-center justify-center bg-cyan-500/20 text-cyan-400"><mod.icon size={20} /></div><span className="font-bold text-lg text-white">{mod.name}</span></div></div>
                              <p className="text-sm text-gray-400 mb-5 flex-grow">{mod.desc}</p>
                              <button onClick={() => setActiveConfigModule(mod.name)} className="w-full py-2 bg-gradient-to-r from-blue-600/20 to-cyan-500/10 text-cyan-400 font-medium rounded-xl border border-cyan-500/20 flex items-center justify-center gap-2"><Sliders size={16} /> Налаштувати</button>
                            </div>
                          ));
                        })()}
                      </div>
                      ...
                      <div className="animate-in slide-in-from-right-8 duration-300 w-full max-w-4xl">
                        {activeConfigModule === 'Лід-магніт' && (
                          <div className="space-y-6">
                            ...
                            <div className="flex flex-wrap gap-2 items-center bg-[#131B2C] p-3 rounded-2xl border border-[#1F2937]">
                                <span className="text-sm font-bold text-gray-400 mr-2 uppercase tracking-wider">Воронки:</span>
                                {(builderForm.moduleConfigs['Лід-магніт']?.flows || []).map(flow => (
                                    <div key={flow.id} className="relative flex items-center"><button onClick={() => setActiveFlowId(flow.id)} className={`px-4 py-1.5 rounded-l-lg text-sm font-medium transition-colors ${activeFlowId === flow.id ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'bg-[#0B1120] text-gray-400 border border-[#1F2937] hover:bg-zinc-800'}`}>{flow.name || flow.trigger}</button><button onClick={() => toggleLmFlow(flow.id)} className={`px-2 py-1.5 rounded-r-lg border-y border-r border-[#1F2937] text-xs font-bold transition-colors ${flow.isActive !== false ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`} title={flow.isActive !== false ? 'Воронка увімкнена. Натисніть, щоб вимкнути' : 'Воронка вимкнена. Натисніть, щоб увімкнути'}>{flow.isActive !== false ? 'ВКЛ' : 'ВИКЛ'}</button></div>
                                ))}
                                <button onClick={addLmFlow} className="px-3 py-1.5 rounded-lg text-sm font-medium bg-blue-600/10 text-cyan-400 border border-cyan-500/20 hover:bg-blue-600/20 transition-colors">+ Додати воронку</button>
                            </div>

                            {activeLmFlow && (
                              <div className="space-y-6">
                                <div className="bg-[#131B2C] p-5 rounded-2xl border border-[#1F2937] flex items-end gap-4 flex-wrap">
                                  <div className="flex-1 min-w-[200px]"><label className="text-sm font-medium text-gray-300 mb-1.5 flex items-center gap-2"><MessageCircle size={16} className="text-cyan-400"/> Назва воронки</label><input type="text" value={activeLmFlow.name} onChange={e => updateLmFlow(activeLmFlow.id, 'name', e.target.value)} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-2.5 text-white focus:border-cyan-500 outline-none" /></div>
                                  <div className="flex-1 min-w-[200px]"><label className="text-sm font-medium text-gray-300 mb-1.5 flex items-center gap-2"><Zap size={16} className="text-cyan-400"/> Тригерне слово (активация)</label><input type="text" value={activeLmFlow.trigger} onChange={e => updateLmFlow(activeLmFlow.id, 'trigger', e.target.value)} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-2.5 text-white focus:border-cyan-500 outline-none" placeholder="наприклад: аудит" /></div>
                                  <button onClick={() => deleteLmFlow(activeLmFlow.id)} className="h-[46px] px-4 rounded-xl bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20 font-medium transition-colors shrink-0">Видалити воронку</button>
                                </div>
                                <div className="space-y-4 relative">
                                    <div className="absolute left-[27px] top-6 bottom-6 w-0.5 bg-[#1F2937] -z-10"></div>
                                    {(activeLmFlow.steps || []).map((step, index) => (
                                        <div key={step.id} className="bg-[#131B2C] rounded-2xl border border-[#1F2937] overflow-hidden shadow-sm relative z-0">
                                            <div className="bg-[#0B1120] px-4 py-3 border-b border-[#1F2937] flex items-center justify-between">
                                                <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-cyan-500/20 text-cyan-400 text-xs font-bold flex items-center justify-center border border-cyan-500/30">{index + 1}</div><span className="font-semibold text-sm text-white">{step.type === 'message' ? 'Повідомлення' : step.type === 'wait_input' ? 'Очікування слова' : step.type === 'check_sub' ? 'Перевірка підписки' : 'Таймер (Затримка)'}</span></div>
                                                <div className="flex items-center gap-3"><span className="text-[10px] text-gray-500 font-mono bg-[#131B2C] px-2 py-1 rounded">ID: {step.id}</span><button onClick={() => deleteLmStep(step.id)} className="text-gray-500 hover:text-red-400 transition-colors"><Trash2 size={16}/></button></div>
                                            </div>
                                            <div className="p-5">
                                                {step.type === 'message' && (
                                                    <div className="space-y-4">
                                                        <div className="flex items-center gap-4 bg-[#0B1120] border border-[#1F2937] p-4 rounded-xl mb-4"><Clock size={20} className="text-amber-400 shrink-0"/><div className="w-full"><label className="text-[11px] font-bold text-gray-400 uppercase block mb-2">Таймер перед відправкою повідомлення:</label><div className="grid grid-cols-4 gap-2"><div className="flex flex-col"><input type="number" min="0" value={step.delayDays || 0} onChange={e => updateLmStep(step.id, 'delayDays', parseInt(e.target.value)||0)} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-2 py-1.5 text-white text-sm text-center outline-none focus:border-amber-500" /><span className="text-[9px] text-gray-500 mt-1 text-center uppercase font-bold">Дні</span></div><div className="flex flex-col"><input type="number" min="0" max="23" value={step.delayHours || 0} onChange={e => updateLmStep(step.id, 'delayHours', parseInt(e.target.value)||0)} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-2 py-1.5 text-white text-sm text-center outline-none focus:border-amber-500" /><span className="text-[9px] text-gray-500 mt-1 text-center uppercase font-bold">Год</span></div><div className="flex flex-col"><input type="number" min="0" max="59" value={step.delayMinutes || 0} onChange={e => updateLmStep(step.id, 'delayMinutes', parseInt(e.target.value)||0)} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-2 py-1.5 text-white text-sm text-center outline-none focus:border-amber-500" /><span className="text-[9px] text-gray-500 mt-1 text-center uppercase font-bold">Хв</span></div><div className="flex flex-col"><input type="number" min="0" max="59" value={step.delaySeconds || 0} onChange={e => updateLmStep(step.id, 'delaySeconds', parseInt(e.target.value)||0)} className="w-full bg-[#131B2C] border border-[#1F2937] rounded-lg px-2 py-1.5 text-white text-sm text-center outline-none focus:border-amber-500" /><span className="text-[9px] text-gray-500 mt-1 text-center uppercase font-bold">Сек</span></div></div></div></div>
                                                        <textarea rows="3" value={step.text} onChange={e => updateLmStep(step.id, 'text', e.target.value)} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-3 text-white focus:border-cyan-500 outline-none resize-none" placeholder="Текст повідомлення..." />
                                                        <div className="flex flex-col sm:flex-row sm:items-center gap-3"><select value={step.mediaType || 'none'} onChange={e => updateLmStep(step.id, 'mediaType', e.target.value)} className="bg-[#0B1120] border border-[#1F2937] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-cyan-500 w-full sm:w-40 shrink-0"><option value="none">Без медіа</option><option value="photo">Фото (URL)</option><option value="video">Відео (URL)</option><option value="document">Файл (URL)</option></select>{step.mediaType !== 'none' && (<div className="flex-grow flex gap-2"><input type="url" value={step.mediaUrl} onChange={e => updateLmStep(step.id, 'mediaUrl', e.target.value)} placeholder="Посилання на файл (https://...)" className="flex-grow bg-[#0B1120] border border-[#1F2937] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" /><button onClick={() => handleFileUpload(step.id)} className="shrink-0 w-10 h-10 bg-blue-600/10 text-cyan-400 rounded-xl flex items-center justify-center hover:bg-blue-600/20 transition-colors" title="Завантажити з пристрою (Для Симулятора)"><Upload size={16}/></button></div>)}</div>
                                                        <div className="pt-4 border-t border-[#1F2937] space-y-3"><label className="text-[11px] font-bold text-gray-500 uppercase">Кнопки під повідомленням</label>{(step.buttons || []).map(btn => (<div key={btn.id} className="flex flex-col sm:flex-row gap-2 items-start bg-[#0B1120] p-3 rounded-xl border border-[#1F2937]"><input type="text" value={btn.title} onChange={e => updateLmButton(step.id, btn.id, 'title', e.target.value)} placeholder="Текст (Мій Instagram, Сайт)" className="w-full sm:w-1/3 bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" /><select value={btn.actionType} onChange={e => updateLmButton(step.id, btn.id, 'actionType', e.target.value)} className="w-full sm:w-1/4 bg-[#131B2C] border border-[#1F2937] rounded-lg px-2 py-2 text-white text-sm outline-none focus:border-cyan-500"><option value="url">🌐 Веб-посилання (URL)</option><option value="step">➡️ Перейти до блоку</option></select>{btn.actionType === 'url' ? (<input type="url" value={btn.url} onChange={e => updateLmButton(step.id, btn.id, 'url', e.target.value)} placeholder="https://instagram.com/..., сайт..." className="w-full sm:flex-grow bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" />) : (<select value={btn.nextStepId} onChange={e => updateLmButton(step.id, btn.id, 'nextStepId', e.target.value)} className="w-full sm:flex-grow bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-cyan-500"><option value="">Оберіть блок...</option>{activeLmFlow.steps.filter(s => s.id !== step.id).map((s, idx) => (<option key={s.id} value={s.id}>{getStepPreviewLabel(s, activeLmFlow.steps.indexOf(s))}</option>))}</select>)}<button onClick={() => deleteLmButton(step.id, btn.id)} className="w-full sm:w-9 h-9 flex items-center justify-center text-gray-500 hover:text-red-400 bg-zinc-800 sm:bg-transparent rounded-lg"><Trash2 size={16} /></button></div>))}<button onClick={() => addLmButton(step.id)} className="w-full py-2 bg-[#0B1120] hover:bg-zinc-800 text-cyan-400 text-sm rounded-xl border border-[#1F2937] transition-colors flex items-center justify-center gap-2"><LinkIcon size={14}/> Додати кнопку розгалуження / Веб-посилання</button></div>
                                                    </div>
                                                )}
                                                {step.type === 'check_sub' && (
                                                  <div className="space-y-4">
                                                      <div className="bg-blue-500/10 border border-blue-500/20 p-4 rounded-xl flex gap-3 items-start">
                                                         ...
                                                         <div className="flex-grow space-y-3">
                                                             <p className="text-[11px] text-blue-300 font-medium">Щоб бот міг перевіряти підписку, він обов'язково має бути <b>Адміністратором</b> у вашому каналі!</p>
                                                             <div>
                                                                 <label className="text-[11px] font-bold text-gray-400 uppercase block mb-1">Посилання на канал (для кнопки):</label>
                                                                 <input type="url" value={step.channelUrl || ''} onChange={e => updateLmStep(step.id, 'channelUrl', e.target.value)} placeholder="https://t.me/..." className="w-full bg-[#0B1120] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-blue-500" />
                                                             </div>
                                                             <div>
                                                                 <label className="text-[11px] font-bold text-gray-400 uppercase block mb-1">ID або Username каналу (для API):</label>
                                                                 <input type="text" value={step.channelId || ''} onChange={e => updateLmStep(step.id, 'channelId', e.target.value)} placeholder="@channel_username або -10012345678" className="w-full bg-[#0B1120] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-blue-500" />
                                                             </div>
                                                             <div className="pt-3 mt-3 border-t border-blue-500/20">
                                                                 <label className="text-[11px] font-bold text-gray-400 uppercase block mb-2">Повідомлення перед перевіркою:</label>
                                                                 <textarea rows="2" value={step.text || ''} onChange={e => updateLmStep(step.id, 'text', e.target.value)} placeholder="Підпишіться на канал, щоб отримати бонус..." className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-3 text-white focus:border-blue-500 outline-none resize-none mb-3" />
                                                                 <div className="flex flex-col sm:flex-row sm:items-center gap-3"><select value={step.mediaType || 'none'} onChange={e => updateLmStep(step.id, 'mediaType', e.target.value)} className="bg-[#0B1120] border border-[#1F2937] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-blue-500 w-full sm:w-40 shrink-0"><option value="none">Без медіа</option><option value="photo">Фото (URL)</option><option value="video">Відео (URL)</option><option value="document">Файл (URL)</option></select>{step.mediaType && step.mediaType !== 'none' && (<div className="flex-grow flex gap-2"><input type="url" value={step.mediaUrl || ''} onChange={e => updateLmStep(step.id, 'mediaUrl', e.target.value)} placeholder="Посилання на файл..." className="flex-grow bg-[#0B1120] border border-[#1F2937] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-blue-500" /><button onClick={() => handleFileUpload(step.id)} className="shrink-0 w-10 h-10 bg-blue-600/20 text-blue-400 rounded-xl flex items-center justify-center hover:bg-blue-600/30 transition-colors" title="Завантажити з пристрою"><Upload size={16}/></button></div>)}</div>
                                                             </div>
                                                             <div className="pt-3 mt-3 border-t border-blue-500/20">
                                                                 <label className="text-[11px] font-bold text-gray-400 uppercase block mb-1">Якщо НЕ підписався (текст помилки):</label>
                                                                 <input type="text" value={step.fallbackText || ''} onChange={e => updateLmStep(step.id, 'fallbackText', e.target.value)} placeholder="Ви не підписались! Спробуйте ще раз." className="w-full bg-[#0B1120] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-red-500" />
                                                             </div>
                                                             <div className="pt-3 mt-3 border-t border-blue-500/20">
                                                                 <label className="text-[11px] font-bold text-green-400 uppercase block mb-1">Якщо ПІДПИСАВСЯ (наступний крок):</label>
                                                                 <select value={step.nextStepId || ''} onChange={e => updateLmStep(step.id, 'nextStepId', e.target.value)} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-green-500">
                                                ...
                                                {step.type === 'wait_input' && (
                                                    <div className="space-y-4"><div className="bg-purple-500/10 border border-purple-500/20 p-4 rounded-xl flex gap-3 items-start"><MessageSquare size={18} className="text-purple-400 mt-0.5 shrink-0"/><div className="flex-grow space-y-3"><div><label className="text-[11px] font-bold text-gray-400 uppercase block mb-1">Чекати поки клієнт напише (Кодове слово):</label><input type="text" value={step.expectedText} onChange={e => updateLmStep(step.id, 'expectedText', e.target.value)} placeholder="Наприклад: переглянув" className="w-full bg-[#0B1120] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-purple-500" /></div><div><label className="text-[11px] font-bold text-gray-400 uppercase block mb-1">Повідомлення про помилку (Якщо написав інше):</label><input type="text" value={step.fallbackText || ''} onChange={e => updateLmStep(step.id, 'fallbackText', e.target.value)} placeholder="Будь ласка, напишіть правильне слово..." className="w-full bg-[#0B1120] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-purple-500" /></div><div className="pt-3 mt-3 border-t border-purple-500/20"><label className="text-[11px] font-bold text-purple-400 uppercase block mb-2">Відповісти при успішному вводі:</label><textarea rows="2" value={step.successText || ''} onChange={e => updateLmStep(step.id, 'successText', e.target.value)} placeholder="Молодець, ось твоє відео..." className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-3 text-white focus:border-purple-500 outline-none resize-none mb-3" /><div className="flex flex-col sm:flex-row sm:items-center gap-3"><select value={step.successMediaType || 'none'} onChange={e => updateLmStep(step.id, 'successMediaType', e.target.value)} className="bg-[#0B1120] border border-[#1F2937] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-purple-500 w-full sm:w-40 shrink-0"><option value="none">Без медіа</option><option value="photo">Фото (URL)</option><option value="video">Відео (URL)</option><option value="document">Файл (URL)</option></select>{step.successMediaType && step.successMediaType !== 'none' && (<div className="flex-grow flex gap-2"><input type="url" value={step.successMediaUrl || ''} onChange={e => updateLmStep(step.id, 'successMediaUrl', e.target.value)} placeholder="Посилання на файл..." className="flex-grow bg-[#0B1120] border border-[#1F2937] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-purple-500" /><button onClick={() => handleFileUpload(step.id, true)} className="shrink-0 w-10 h-10 bg-purple-600/20 text-purple-400 rounded-xl flex items-center justify-center hover:bg-purple-600/30 transition-colors" title="Завантажити з пристрою"><Upload size={16}/></button></div>)}</div></div></div></div></div>
                                                )}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-6 border-t border-[#1F2937]">
                                    <button onClick={() => addLmStep('message')} className="py-3 bg-[#131B2C] hover:bg-[#1E293B] border border-[#1F2937] rounded-xl text-white font-medium text-sm transition-colors shadow-sm flex items-center justify-center gap-2"><MessageSquare size={16} className="text-cyan-400"/> Додати повідомлення</button>
                                    <button onClick={() => addLmStep('check_sub')} className="py-3 bg-[#131B2C] hover:bg-[#1E293B] border border-[#1F2937] rounded-xl text-white font-medium text-sm transition-colors shadow-sm flex items-center justify-center gap-2"><ShieldCheck size={16} className="text-blue-400"/> Перевірка підписки</button>
                                    <button onClick={() => addLmStep('wait_input')} className="py-3 bg-[#131B2C] hover:bg-[#1E293B] border border-[#1F2937] rounded-xl text-white font-medium text-sm transition-colors shadow-sm flex items-center justify-center gap-2"><Command size={16} className="text-purple-400"/> Очікувати текст</button>
                                </div>
                              </div>
                            )}
                          </div>
                        ...
                        {activeConfigModule === 'Автоворонка' && (
                          <div className="space-y-5">
                            <div className="bg-[#131B2C] p-5 rounded-2xl border border-[#1F2937]"><label className="text-sm font-medium text-gray-300 mb-1.5 flex items-center gap-2"><Zap size={16} className="text-cyan-400"/> Тригер запуску</label><input type="text" value={builderForm.moduleConfigs['Автоворонка'].trigger || ''} onChange={e => updateModuleConfig('Автоворонка', 'trigger', e.target.value)} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-2.5 text-white focus:border-cyan-500 outline-none" placeholder="/start" /></div>
                            
                            {/* НОВИЙ БЛОК: ПЕРЕВІРКА ПІДПИСКИ */}
                            <div className="bg-[#131B2C] p-5 rounded-2xl border border-[#1F2937]">
                                <div className="flex items-center justify-between mb-4">
                                    <label className="text-sm font-medium text-white flex items-center gap-2"><ShieldCheck size={16} className="text-blue-400"/> Обов'язкова підписка на Телеграм канал</label>
                                    <div className={`w-12 h-6 rounded-full p-1 cursor-pointer transition-colors ${builderForm.moduleConfigs['Автоворонка'].requireSub ? 'bg-cyan-500' : 'bg-gray-600'}`} onClick={() => updateModuleConfig('Автоворонка', 'requireSub', !builderForm.moduleConfigs['Автоворонка'].requireSub)}>
                                ...
                                {builderForm.moduleConfigs['Автоворонка'].requireSub && (
                                    <div className="space-y-3 pt-3 border-t border-[#1F2937]">
                                        <p className="text-[11px] text-blue-300 font-medium mb-2">Бот має бути Адміністратором у вашому каналі!</p>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div><label className="text-[11px] font-bold text-gray-400 uppercase block mb-1">Посилання (URL) на канал:</label><input type="url" value={builderForm.moduleConfigs['Автоворонка'].channelUrl || ''} onChange={e => updateModuleConfig('Автоворонка', 'channelUrl', e.target.value)} placeholder="https://t.me/..." className="w-full bg-[#0B1120] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-blue-500" /></div>
                                            <div><label className="text-[11px] font-bold text-gray-400 uppercase block mb-1">ID каналу (для API):</label><input type="text" value={builderForm.moduleConfigs['Автоворонка'].channelId || ''} onChange={e => updateModuleConfig('Автоворонка', 'channelId', e.target.value)} placeholder="@channel_username" className="w-full bg-[#0B1120] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-blue-500" /></div>
                                        </div>
                                        <div><label className="text-[11px] font-bold text-gray-400 uppercase block mb-1">Текст з проханням підписатися:</label><input type="text" value={builderForm.moduleConfigs['Автоворонка'].subCheckText || ''} onChange={e => updateModuleConfig('Автоворонка', 'subCheckText', e.target.value)} placeholder="Підпишіться на канал..." className="w-full bg-[#0B1120] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-blue-500" /></div>
                                        <div><label className="text-[11px] font-bold text-gray-400 uppercase block mb-1">Помилка (якщо не підписався):</label><input type="text" value={builderForm.moduleConfigs['Автоворонка'].subErrorText || ''} onChange={e => updateModuleConfig('Автоворонка', 'subErrorText', e.target.value)} placeholder="Ви не підписані!" className="w-full bg-[#0B1120] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-red-500" /></div>
                                    </div>
                                )}
                            </div>

                            <div className="bg-[#131B2C] p-5 rounded-3xl border border-[#1F2937]">
                                <h3 className="text-white font-medium mb-4 flex items-center gap-2 border-b border-[#1F2937] pb-3"><MessageSquare size={16} className="text-cyan-400"/> Відповідь бота (Базова)</h3>
                                {(() => { const steps = builderForm.moduleConfigs['Автоворонка'].steps; const step = steps.length > 0 ? steps[0] : { id: 1, text: '', links: [] }; return ( <div className="space-y-5"><div><label className="text-[11px] uppercase text-gray-500 block mb-2 font-bold">Текст повідомлення</label><textarea rows="4" value={step.text || ''} onChange={e => updateStarterFunnel('text', null, e.target.value)} className="w-full bg-[#0B1120] border border-[#1F2937] rounded-xl px-4 py-3 text-white focus:border-cyan-500 outline-none resize-none" placeholder="Що відповість бот..." /></div><div className="pt-2"><label className="text-[11px] uppercase text-gray-500 block mb-2 font-bold">Веб-посилання під повідомленням (Кнопки)</label>{(step.links || []).map((link, idx) => (<div key={link.id || idx} className="flex gap-2 mb-2 items-start"><input type="text" value={link.title} onChange={e => updateStarterFunnelLinks('update', idx, 'title', e.target.value)} placeholder="Назва (напр. Мій Instagram)" className="w-1/3 bg-[#0B1120] border border-[#1F2937] rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-cyan-500" /><input type="url" value={link.url} onChange={e => updateStarterFunnelLinks('update', idx, 'url', e.target.value)} placeholder="Посилання (https://...)" className="flex-1 bg-[#0B1120] border border-[#1F2937] rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-cyan-500" /><button onClick={() => updateStarterFunnelLinks('delete', idx)} className="w-10 h-10 shrink-0 flex items-center justify-center text-gray-500 hover:text-red-400 bg-[#0B1120] border border-[#1F2937] rounded-xl"><Trash2 size={16}/></button></div>))}<button onClick={() => updateStarterFunnelLinks('add')} className="w-full py-2.5 bg-blue-600/10 hover:bg-blue-600/20 text-cyan-400 text-sm font-medium rounded-xl border border-cyan-500/20 transition-colors flex items-center justify-center gap-2"><LinkIcon size={16}/> Додати веб-посилання</button></div></div> ); })()}
                            </div>
                          </div>
                        )}
                      </div>
                ...
                {builderTab === 'menu' && (
                  <div className="flex flex-col lg:flex-row gap-6 h-full animate-in fade-in duration-300">
                    <div className="flex-1 bg-[#131B2C] border border-[#1F2937] p-5 rounded-3xl h-full flex flex-col">
                      <div className="mb-4 flex justify-between items-start"><div><h3 className="text-lg font-bold text-white flex items-center gap-2"><Command size={18} className="text-cyan-400"/> Меню бота</h3></div><button onClick={loadSmmMenuPreset} className="px-3 py-1.5 bg-[#0B1120] border border-[#1F2937] rounded-lg text-xs text-cyan-400">Шаблон</button></div>
                          ...
                          <div key={idx} className="flex flex-col gap-2 bg-[#0B1120] p-3 rounded-2xl border border-[#1F2937]">
                              <div className="flex gap-2 items-start">
                                  <div className="flex-1 relative">
                                      ...
                                      <input type="text" value={item.command} onChange={e => updateMenuCommand(idx, 'command', e.target.value)} placeholder="команда" className="w-full bg-[#131B2C] border border-[#1F2937] rounded-xl pl-6 pr-2 py-2 text-white font-mono text-sm outline-none focus:border-cyan-500" />
                                  </div>
                                  <input type="text" value={item.description} onChange={e => updateMenuCommand(idx, 'description', e.target.value)} placeholder="Опис (в меню)" className="flex-[2] bg-[#131B2C] border border-[#1F2937] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" />
                                  <button onClick={() => removeMenuCommand(idx)} className="w-9 h-9 flex items-center justify-center text-gray-500 hover:text-red-400 bg-[#131B2C] border border-[#1F2937] rounded-xl"><Trash2 size={16}/></button>
                              </div>
                              <div className="pt-2 border-t border-[#1F2937] mt-1">
                                  <label className="text-[11px] font-bold text-gray-500 uppercase mb-1.5 block">Відповідь бота (Текст та Медіа)</label>
                                  <textarea rows="2" value={item.message || ''} onChange={e => updateMenuCommand(idx, 'message', e.target.value)} placeholder="Текст повідомлення..." className="w-full bg-[#131B2C] border border-[#1F2937] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-cyan-500 resize-none mb-2" />
                                  <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-3">
                                      <select value={item.mediaType || 'none'} onChange={e => updateMenuCommand(idx, 'mediaType', e.target.value)} className="bg-[#131B2C] border border-[#1F2937] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-cyan-500 w-full sm:w-40 shrink-0">
                                      ...
                                      {item.mediaType && item.mediaType !== 'none' && (
                                          <div className="flex-grow flex gap-2">
                                              <input type="url" value={item.mediaUrl || ''} onChange={e => updateMenuCommand(idx, 'mediaUrl', e.target.value)} placeholder="Посилання на файл..." className="flex-grow bg-[#131B2C] border border-[#1F2937] rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-cyan-500" />
                                          </div>
                                      )}
                                  </div>
                                  <label className="text-[11px] font-bold text-gray-500 uppercase mb-1.5 block">Веб-посилання (Кнопки під відповіддю)</label>
                                  {(item.links || []).map((link, lidx) => (
                                      <div key={link.id || lidx} className="flex gap-2 mb-2 items-start">
                                          <input type="text" value={link.title} onChange={e => handleMenuLink('update', idx, lidx, 'title', e.target.value)} placeholder="Назва" className="w-1/3 bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-xs outline-none focus:border-cyan-500" />
                                          <input type="url" value={link.url} onChange={e => handleMenuLink('update', idx, lidx, 'url', e.target.value)} placeholder="https://..." className="flex-1 bg-[#131B2C] border border-[#1F2937] rounded-lg px-3 py-2 text-white text-xs outline-none focus:border-cyan-500" />
                                          <button onClick={() => handleMenuLink('delete', idx, lidx)} className="w-8 h-8 shrink-0 flex items-center justify-center text-gray-500 hover:text-red-400 bg-[#131B2C] border border-[#1F2937] rounded-lg"><Trash2 size={14}/></button>
                                      </div>
                                  ))}
                                  <button onClick={() => handleMenuLink('add', idx)} className="w-full py-2 bg-[#131B2C] hover:bg-[#1E293B] text-cyan-400 text-xs font-medium rounded-lg border border-[#1F2937] transition-colors flex items-center justify-center gap-1 mt-1"><LinkIcon size={12}/> Додати посилання</button>
                              </div>
                          </div>
                        ))}
                      </div>
                      <button onClick={addMenuCommand} className="w-full py-3 mt-auto bg-blue-600/10 border border-cyan-500/20 rounded-xl text-cyan-400 text-sm">Додати команду</button>
                    </div>

                    {/* РАБОЧИЙ СИМУЛЯТОР ТЕЛЕФОНА */}
                    <div className="w-full lg:w-[340px] bg-[#8BA7B6] rounded-[2.5rem] border-[10px] border-[#0A0F1D] shadow-[0_0_40px_rgba(0,0,0,0.5)] flex flex-col shrink-0 h-[600px] lg:h-[650px] overflow-hidden relative">
                        <div className="bg-[#17212B] px-3 py-2 flex items-center gap-3 shrink-0 mt-6 shadow-md relative z-20"><ChevronLeft size={24} className="text-[#5288C1]" /><div className="w-9 h-9 rounded-full bg-cyan-400 flex justify-center items-center"><Bot size={20} className="text-white"/></div><div><div className="text-white font-medium text-[15px] truncate w-32">{builderForm.name || 'Бот'}</div><div className="text-[#5288C1] text-[12px]">bot</div></div></div>
                        <div className="flex-grow bg-[#0E1621] relative overflow-y-auto p-3 space-y-3 scrollbar-none pb-[80px]">
                            <div className="text-center text-[#5288C1] text-xs my-2 font-medium">Сьогодні</div>
                            {previewChat.map((msg, i) => (
                                <div key={i} className={`flex flex-col max-w-[85%] ${msg.sender === 'bot' ? 'self-start items-start' : 'self-end items-end ml-auto'}`}>
                                    <div className={`p-3 rounded-2xl text-[15px] shadow-sm relative ${msg.sender === 'bot' ? 'bg-[#182533] text-white rounded-bl-sm' : 'bg-[#2B5278] text-white rounded-br-sm'}`}>
                                        {msg.mediaType === 'photo' && <div className="w-full h-32 bg-[#0E1621] rounded-lg mb-2 flex items-center justify-center text-[#5288C1] border border-[#1F2937]"><Upload size={24}/></div>}
                                        {msg.mediaType === 'video' && <div className="w-full h-32 bg-[#0E1621] rounded-lg mb-2 flex items-center justify-center text-[#5288C1] border border-[#1F2937] relative"><Play size={24}/><div className="absolute bottom-2 right-2 bg-black/50 text-[10px] px-1 rounded">0:15</div></div>}
                                        {msg.mediaType === 'document' && <div className="w-full bg-[#0E1621] rounded-lg p-2 mb-2 flex items-center gap-3 border border-[#1F2937]"><div className="w-10 h-10 bg-[#2B5278] rounded-full flex items-center justify-center"><FileText size={20} className="text-white"/></div><div className="flex-1"><p className="text-sm font-medium truncate">file.pdf</p><p className="text-xs text-[#5288C1]">2.4 MB</p></div></div>}
                                        <p className="whitespace-pre-wrap leading-tight">{msg.text}</p>
                                        <span className={`text-[10px] float-right mt-1 ml-2 ${msg.sender === 'bot' ? 'text-[#5288C1]' : 'text-blue-300'}`}>12:00</span>
                                    </div>
                                    {msg.buttons && msg.buttons.length > 0 && (
                                        <div className="flex flex-col gap-1 w-full mt-1">
                                            {msg.buttons.map((btn, idx) => (
                                                <button key={idx} onClick={() => handleSimulatorButton(btn, msg.currentFlowId)} className="w-full bg-[#182533] text-[#5288C1] hover:bg-[#202E3D] py-2 rounded-xl text-sm font-medium transition-colors border border-[#1F2937]/50 shadow-sm flex justify-center items-center gap-1">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-[#17212B] p-2 flex items-center gap-2 border-t border-[#0A0F1D]/50 z-20 pb-8 lg:pb-2">
                            <button className="text-[#5288C1] p-2 hover:bg-white/5 rounded-full transition-colors"><Upload size={24} /></button>
                            <input type="text" value={previewInput} onChange={e => setPreviewInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendPreviewMsg()} placeholder="Повідомлення..." className="flex-grow bg-[#0E1621] text-white rounded-full px-4 py-2 outline-none text-[15px]" />
                            {previewInput.trim() ? (
                                <button onClick={() => sendPreviewMsg()} className="w-9 h-9 rounded-full bg-[#5288C1] flex items-center justify-center text-white hover:bg-[#6398D1] transition-colors"><Send size={16} className="-ml-0.5" /></button>
                            ) : (
                                <button className="text-[#5288C1] p-2 hover:bg-white/5 rounded-full transition-colors"><MessageCircle size={24} /></button>
                            )}
                        </div>
                    </div>
                  </div>
